...
# ship-instructions.md in the task's grouped durable data directory, appends
# that same superseding contract to brief.md there for future relaunches, and
# prints the fm-send.sh command that delivers it to the current worker.
# Those instructions carry the scratch-state inventory, the clean
...
TASK_DIR=$(fm_task_data_ensure_dir "$DATA" "$ID") || {
...
Your CI rule now reads, for this mode: $CI_RULE
...
BRIEF_REPLACEMENT="$TASK_DIR/.brief.md.promote.${BASHPID:-$$}"
{
  cat "$SCOUT_BRIEF"
  printf '\n\n'
...
BRIEF_ORIGINAL="$TASK_DIR/.brief.md.scout.${BASHPID:-$$}"
