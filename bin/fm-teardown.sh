...
BACKLOG_DONE_ARGS=()
backlog_done_args() {
  local data_absolute data_relative report_dir report_relative
  BACKLOG_DONE_ARGS=()
  case "$KIND" in
    scout)
      data_absolute=$(fm_backlog_data_absolute "$DATA") || return 1
      data_relative=$(fm_backlog_data_relative "$data_absolute") || return 1
      report_dir=$(fm_task_data_dir "$data_absolute" "$ID") || return 1
      case "$report_dir" in
        "$data_absolute"/*) report_relative=${report_dir#"$data_absolute"/} ;;
        *) return 1 ;;
      esac
      BACKLOG_DONE_ARGS=(--report "$data_relative/$report_relative/report.md")
      ;;
    *)
