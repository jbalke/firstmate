...
# acknowledgement), and the status file itself.
rovo_config_override_flag() {
  local effort=$1 brief_dir=$2 state_dir=$3 id=$4
  local brief_real state_real agent_json paths_json config_json
...
if [ "$KIND" = ship ] || [ "$KIND" = scout ]; then
  ...
  SOURCE_BRIEF=$BRIEF
  # Keep the launch overlay beside its source brief so it follows the task data
  # layout (bin/fm-task-data-lib.sh) rather than the legacy flat folder.
  BRIEF="$(dirname -- "$SOURCE_BRIEF")/launch-brief.md"
  BRIEF_TMP="$(dirname -- "$SOURCE_BRIEF")/.launch-brief.md.${BASHPID:-$$}"
  {
    cat "$SOURCE_BRIEF" &&
      printf '\n' &&
...
if [ "$HARNESS" = rovo ]; then
  ROVOCONFIGOVERRIDE=$(rovo_config_override_flag "$EFFORT" "$BRIEF_DIR_REAL" "$STATE" "$ID") || {
