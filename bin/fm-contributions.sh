...
# owned. Previously observed URLs remain in the task-data directory after
...
# Refuse links at every component beneath the data root before reading or writing.
safe_record_path() {
  local path=$1
  while [ "$path" != "$DATA" ]; do
    case "$path" in "$DATA"/*) ;; *) return 1 ;; esac
    [ ! -L "$path" ] || return 1
    path=$(dirname "$path")
  done
  [ ! -L "$DATA" ]
...
read_saved() {
  local file dir task selected
  : > "$TMP/saved.jsonl"
  ERRORS=0
  if [ -L "$DATA" ]; then
    ERRORS=1; printf '[]\n' > "$TMP/saved.json"; return 0
  fi
  for file in "$DATA/$FM_TASK_DATA_SUBDIR"/*/*/contributions.json "$DATA"/*/contributions.json; do
    [ -e "$file" ] || [ -L "$file" ] || continue
    dir=$(dirname "$file")
    task=$(basename "$dir")
    selected=$(fm_task_data_find "$DATA" "$task") || { ERRORS=$((ERRORS + 1)); continue; }
    [ "$dir" = "$selected" ] || continue
    if ! safe_record_path "$file" || [ ! -f "$file" ] \
