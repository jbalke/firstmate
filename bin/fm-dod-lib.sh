...
# fm_dod_block <no-mistakes|direct-PR|local-only> <task-id> <task-data-dir>
# prints the block on stdout with no trailing blank line. The caller validates
# the mode and task data directory; an unknown mode is refused rather than
...
fm_ask_user_escalation_block() {  # <task-data-dir>
  local task_dir=$1
  cat <<EOF
   For a no-mistakes ask-user gate specifically, escalate all ask-user findings as one event plus one snapshot file, using that same shape even when the gate holds only a single ask-user finding: write only the ask-user findings, verbatim and unparaphrased (id, severity, file, line, description, authority), to \`$task_dir/nm-<run>-findings.txt\`, then report the gate with
   \`needs-decision [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=$task_dir/nm-<run>-findings.txt\`
