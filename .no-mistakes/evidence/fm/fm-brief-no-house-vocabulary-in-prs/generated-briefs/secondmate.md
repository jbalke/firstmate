...
Firstmate steers you through durable message files in '<HOME>/state/brief-ev-secondmate.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '<HOME>/state/brief-ev-secondmate.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '<HOME>/state/brief-ev-secondmate.inbox'/NNN.msg '<HOME>/state/brief-ev-secondmate.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Escalation to main firstmate
Handle routine work yourself.
Report only true captain-relevant outcomes or a declared external wait by appending one line:
   `echo "{state}: {one short line}" >> '<HOME>/state/brief-ev-secondmate.status'`
