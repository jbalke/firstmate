...
1. First action: create your branch: `git checkout -b fm/brief-ev-direct-PR`

# Rules
1. Never push to the default branch (push only your `fm/brief-ev-direct-PR` branch). Never merge a PR.
2. Stay inside this worktree; the only files you may write outside it are the status file in rule 4, your instruction inbox acknowledgements below, and anything under your task data directory `<HOME>/data/tasks/some-proj/brief-ev-direct-PR/` (the report and saved evidence below).
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '<HOME>/state/brief-ev-direct-PR.status'`
...
Firstmate steers you through durable message files in '<HOME>/state/brief-ev-direct-PR.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '<HOME>/state/brief-ev-direct-PR.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '<HOME>/state/brief-ev-direct-PR.inbox'/NNN.msg '<HOME>/state/brief-ev-direct-PR.inbox'/handled/`.
...
## Durable report
Write `<HOME>/data/tasks/some-proj/brief-ev-direct-PR/report.md` if this task uncovered a **transferable cause** - something that would bite a future task on this repo or another.
...
## Saved evidence
Write any screenshot or recording you take to support a claim in the PR body into `<HOME>/data/tasks/some-proj/brief-ev-direct-PR/`, naming each file for what it shows.
