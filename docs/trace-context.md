...
- **Fail-independent.**
  ...
  If recording the carrier fails after export, Firstmate clears `TRACEPARENT` in the launch command with `env -u` (never a shell builtin, which the operator's login shell may not have) and still launches the task, so the child never receives an identity absent from its metadata.
- **Metadata-only.**
