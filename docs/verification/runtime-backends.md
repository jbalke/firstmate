...
After the fix, Codex single-dot particle-only rows bound the wrap region (the status footer sits beneath the starfield row, so the region never reaches it), single-dot particle-only content behind the placeholder is recognized on the glyph row, and the same capture reads `empty` under the Herdr and Zellij styled profiles and with a tmux cursor on the glyph row, while a plain (`styled=0`) capture still reads `unknown`, never `pending`.
...
- Codex 0.154's idle starfield rows are composer furniture only because they carry nothing but its eight single-dot Braille particles, so any other Braille, progress spinners included, stays typed content; the dated Herdr evidence and refresh command are in [Composer classification matrix](#composer-classification-matrix).
