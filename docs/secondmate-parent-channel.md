...
| Scout child findings | the child's `done:` line plus the resolved report pointer (`data/tasks/<project>/<child>/report.md` under the current layout, or `data/<child>/report.md` for a legacy flat folder) | `bin/fm-inactive-reconcile.sh` on the next poll, with the report pointer |
