...
It also owns the CI-following rule each contract carries, a scout's included, so the brief scaffold and a promotion state that rule from one place and a promoted worker is never left with pre-promotion wording its new definition of done contradicts.
