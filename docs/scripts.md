...
| [`fm-dod-lib.sh`](../bin/fm-dod-lib.sh) | Own ship/scout worker role scope, ship definitions of done, their shared repo-artifact rules, each contract's CI-following rule rendered by both the brief scaffold and a scout promotion, and the no-mistakes `--intent` contract |
