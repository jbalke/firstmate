...
On 0 only the close changes: after cleanup and still under the task's own lock, teardown records one `Deliverable of the finished work: ...` line at the end of the task body, copies a supported pull request or `data/`-relative report into the row's structured artifact fields, and runs `tasks-axi reopen`, so the row returns to Queued with its hold intact and remains on the appropriate Captain's Call or Charted Next decision surface instead of reading as work still under way.
...
A retained report is not similarly bounded: tasks-axi validates `--report` against `/\bdata\/\S+?\/report\.md\b/`, so it accepts any depth under `data/` - the flat `data/<id>/report.md`, the project-grouped `data/tasks/<project>/<id>/report.md` this home scaffolds (bin/fm-task-data-lib.sh), and any other `data/`-relative folder a report was moved to.
`fm_backlog_row_artifact_supported` accepts that same shape, minus a traversal and minus whitespace: the validator also admits `data/../etc/report.md`, which firstmate refuses to write onto a durable row, and a shell glob does not stop at whitespace where the validator's `\S+?` does.
The predicate names the whitespace it refuses - space, tab, newline, vertical tab, form feed, carriage return and U+00A0 - rather than matching a `[[:space:]]` class whose members depend on the ambient locale, so the refusal is the same under a C/POSIX locale as under a UTF-8 one and nothing outside that set is refused.
A report that reaches neither shape - one under a data directory the home renamed, which the validator rejects outright because the literal `data/` prefix is missing, or one carrying whitespace, which the validator rejects for the same reason the predicate does - still reaches the task body as its `Deliverable of the finished work: ...` line; only the structured row artifact is skipped.
When an interrupted retention leaves such a report in the validated pending-close record, `answer` skips only that known-unsupported row artifact and closes normally, so the delivery remains absent from Recently Landed instead of wedging the captain's answer.
