...
`fm_backlog_row_artifact_supported` accepts that same shape, minus a traversal: the validator also admits `data/../etc/report.md`, which firstmate refuses to write onto a durable row.
A report that reaches neither shape - one under a data directory the home renamed, which the validator rejects outright because the literal `data/` prefix is missing - still reaches the task body as its `Deliverable of the finished work: ...` line; only the structured row artifact is skipped.
