...
test_completion_closes_a_scout_with_its_report() {
  local case_dir id out report_dir
  id=atomic-close-b6
  ...
  report_dir="$(home_of "$case_dir")/data/tasks/firstmate/$id"
  mkdir -p "$report_dir"
  ...
  assert_grep "data/tasks/firstmate/$id/report.md" "$(backlog_of "$case_dir")" \
...
test_completion_records_a_relative_report_for_relocated_data() {
  local case_dir id relocated backlog out report_dir
  ...
  report_dir="$relocated/tasks/firstmate/$id"
  mkdir -p "$report_dir"
  printf 'findings\n' > "$report_dir/report.md"
  FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$(home_of "$case_dir")" \
  ...
  assert_grep "data/tasks/firstmate/$id/report.md" "$backlog" \
