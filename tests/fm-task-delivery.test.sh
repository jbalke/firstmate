...
test_promote_requires_and_records_the_delivery_contract() {
  ...
  instructions_path="$home/data/tasks/_none/promote-d1/ship-instructions.md"
...
test_promote_refuses_a_symlinked_task_record() {
  ...
  assert_absent "$home/data/tasks/_none/promote-sym/ship-instructions.md" \
...
STUB
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    fill_brief_subsections "$(fm_test_task_brief "$home" "$id")" \
    ...
    awk '/^# Definition of done$/ { emit=1 } emit' \
      "$(fm_test_task_brief "$home" "$id")" > "$brief_dod"
    ...
    assert_grep "$(dirname "$(fm_test_task_brief "$home" "$id")")/report.md" "$payload" \
      "$mode: promoted worker received the wrong durable report path"
  done
  ...
  assert_grep 'needs-decision [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file='"$(dirname "$(fm_test_task_brief "$home" promote-dod-no-mistakes)")/nm-<run>-findings.txt" "$payload" \
...
EOF
  ...
  assert_present "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md" \
  ...
  assert_grep "supersedes every earlier brief instruction about constructing \`--intent\`" \
    "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md" \
  ...
  authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit && /^$/ { exit } emit { print }' "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md")
...
EOF
  ...
  brief="$(dirname "$(fm_test_task_brief "$home" "$id")")/ship-instructions.md"
...
EOF
  ...
  fill_brief_subsections "$(fm_test_task_brief "$home" "$id")" "$words" 'This build constraint must not become intent.'
  out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
  assert_present "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md" "plain intent was not serialized"
  authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit { print }' "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md")
  ...
  printf '# Task\n## Captain'"'"'s intent\n%s\n\n## Firstmate spec\nDo not paraphrase.\n\n# Definition of done\nDelivery contract: mode=no-mistakes\n' "$words" > "$(fm_test_task_brief "$home" intent-literal)"
  ...
  authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit { print }' "$(dirname "$(fm_test_task_brief "$home" intent-literal)")/launch-brief.md")
  ...
  for marker in 'Captain:' "Captain's words:" "Captain's ask:" "Captain's intent:" 'Captain,'; do
    ...
    printf '# Task\n## Captain'"'"'s intent\nKeep the original request intact.\n  %s preserve its provenance.\n\n## Firstmate spec\nDo not paraphrase.\n\n# Definition of done\nDelivery contract: mode=no-mistakes\n' \
      "$marker" > "$(fm_test_task_brief "$home" "$id")"
    out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
    status=$?
    ...
    assert_absent "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md" "$marker: addressed intent was serialized"
    assert_absent "$home/state/$id.meta" "$marker: addressed intent spawn wrote task metadata"
    assert_grep "  $marker preserve its provenance." "$(fm_test_task_brief "$home" "$id")" "$marker: refusal rewrote the brief"
  done

  id='intent-addressed-promote'
  printf 'window=fm-%s\nkind=scout\nworktree=/tmp/wt\n' "$id" > "$home/state/$id.meta"
  write_brief "$home" "$id"
  printf '# Task\n## Captain'"'"'s intent\nCaptain: investigate the refusal.\n\n## Firstmate spec\nReproduce it first.\n' > "$(fm_test_task_brief "$home" "$id")"
  out=$(FM_HOME="$home" FM_STATE_OVERRIDE="$home/state" "$PROMOTE" "$id" --mode no-mistakes --yolo off 2>&1)
  status=$?
  ...
  assert_absent "$(dirname "$(fm_test_task_brief "$home" "$id")")/ship-instructions.md" "promotion published addressed intent"
  ...
  for marker in '[captain]' 'Captain:' "Captain's words:" "Captain's ask:" "Captain's intent:"; do
    ...
    printf '# Task\n%s %s\nDo not include this build constraint.\n%s %s\n\n# Definition of done\nDelivery contract: mode=no-mistakes\n' \
      "$marker" 'Keep the original request intact.' "$marker" 'Preserve its provenance.' > "$(fm_test_task_brief "$home" "$id")"
    out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
    assert_present "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md" "$marker: provenance was not accepted"
    authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit { print }' "$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md")
...
EOF
  ...
  for project_kind in firstmate unrelated; do
    ...
    for kind in no-mistakes direct-PR local-only scout; do
      ...
      brief="$(dirname "$(fm_test_task_brief "$home" "$id")")/launch-brief.md"
      ...
      assert_no_grep '# Current worker role contract' "$(fm_test_task_brief "$home" "$id")" "spawn rewrote the source brief"
