...
test_matrix_codex_idle_starfield_furniture() {
  ...
  stripped=${stripped#›}
  _fm_composer_row_is_codex_particles "$stripped" \
    || fail "everything surviving ghost stripping behind the glyph must be Codex particles, got '$stripped'"
  ...
  # Cursor-proven Codex particle padding preserves the local empty verdict.
  assert_screen "codex 0.154 cursor on the starfield row" empty "$CAPS_TMUX" "$screen" 4
