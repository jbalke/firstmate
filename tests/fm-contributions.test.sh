...
test_ready_issue_wake() {
  ...
  registered_checks "$home" >/dev/null
  if [ ! -f "$(fm_task_data_find "$home/data" filed)/contributions.json" ] \
    || ! jq -e 'any(.records[].pending[]; .type == "ready-for-pr")' "$(fm_task_data_find "$home/data" filed)/contributions.json" >/dev/null; then
  ...
  for mode in ok fail head; do
    ...
    for task in delivery duplicate; do
      jq -e --arg now "$NOW" --argjson error "$expected" '.records[0].checked_at == $now and .records[0].error == $error' \
        "$(fm_task_data_find "$home/data" "$task")/contributions.json" >/dev/null || fail "owner $task did not receive the shared result ($mode)"
    done
  done
...
test_late_owner_inherits_terminal_observation() {
  ...
  jq -e --slurpfile final "$home/final.json" '
    ...
    "$(fm_task_data_find "$home/data" duplicate)/contributions.json" >/dev/null \
...
test_late_owner_keeps_failure_episode_suppressed() {
  ...
  for task in delivery duplicate; do
    jq -e --arg error "$error" '.records[0].error == $error' "$(fm_task_data_find "$home/data" "$task")/contributions.json" >/dev/null \
  ...
  for task in delivery duplicate; do
    jq -e '.records[0].error == null' "$(fm_task_data_find "$home/data" "$task")/contributions.json" >/dev/null \
...
test_grouped_records_and_legacy_fallback() {
  local home grouped
  home=$(new_home grouped)
  forge_home "$home"
  grouped="$home/data/tasks/sample/delivery"
  mkdir -p "$grouped"
  cp "$home/data/delivery/contributions.json" "$grouped/contributions.json"
  mutate_record "$home" delivery '.records[0].observation.state="merged"'
  with_home "$home" "$ROOT/bin/fm-contributions.sh" poll >/dev/null || fail 'grouped poll failed'
  jq -e '.records[0].observation.state == "open"' "$grouped/contributions.json" >/dev/null \
    || fail 'grouped record did not take precedence over legacy'
  jq -e '.records[0].observation.state == "merged"' "$home/data/delivery/contributions.json" >/dev/null \
    || fail 'grouped write changed legacy record'
  with_home "$home" "$ROOT/bin/fm-contributions.sh" pending | jq -e 'length == 0' >/dev/null \
    || fail 'grouped pending read failed'
  printf -- '- [ ] fresh - Filed https://github.com/o/r/pull/8 (repo: sample) (kind: ship)\n' >> "$home/data/backlog.md"
  with_home "$home" "$ROOT/bin/fm-contributions.sh" poll >/dev/null || fail 'new grouped record poll failed'
  [ -f "$home/data/tasks/sample/fresh/contributions.json" ] || fail 'new task used flat storage'
  [ ! -e "$home/data/fresh" ] || fail 'new task split storage'
  cp "$home/data/delivery/contributions.json" "$home/legacy.json"
  printf 'invalid ignored legacy record\n' > "$home/data/delivery/contributions.json"
  with_home "$home" "$ROOT/bin/fm-contributions.sh" pending >/dev/null \
    || fail 'ignored legacy record poisoned grouped reads'
  mv "$home/legacy.json" "$home/data/delivery/contributions.json"
  rm -rf "$home/data/tasks"
  with_home "$home" "$ROOT/bin/fm-contributions.sh" pending >/dev/null || fail 'legacy fallback failed'
...
test_grouped_symlink_refusal() {
  ...
  for level in tasks project task record; do
    home=$(new_home "symlink-$level")
    forge_home "$home"
    mkdir -p "$home/data/tasks/sample/delivery"
    mv "$home/data/delivery/contributions.json" "$home/data/tasks/sample/delivery/contributions.json"
    rmdir "$home/data/delivery"
    case "$level" in
      tasks) linked="$home/data/tasks" ;;
      project) linked="$home/data/tasks/sample" ;;
      task) linked="$home/data/tasks/sample/delivery" ;;
      record) linked="$home/data/tasks/sample/delivery/contributions.json" ;;
    esac
    target="$home/linked-target"
    mv "$linked" "$target"
    ln -s "$target" "$linked"
    if with_home "$home" "$ROOT/bin/fm-contributions.sh" pending >/dev/null 2>&1; then
      fail "pending accepted symlink at $level"
    fi
    if with_home "$home" "$ROOT/bin/fm-contributions.sh" poll >/dev/null 2>&1; then
      fail "poll wrote through symlink at $level"
    fi
  done
  pass 'grouped contribution paths refuse symlinks at every new level'
}
